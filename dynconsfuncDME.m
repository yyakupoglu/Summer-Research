...
for i = 1: N
    ...
    %DME constraint setup
    iM{i}=inv(massMatrix(robot,q));
    C{i} = velocityProduct(robot,q,v);
    h{i}=h;
...
for i =1:N
        ...
        if i == 1
           Jdot = (J0 -Jc{1})/h0
           rbias = Jc{1}*iM{1}*C{1}+Jc{1}*iM{1}*gv-Jdot*v;
           c=[c;(r-rbias)'*inv((Jc{1}*iM{1})*(Jc{1}*iM{1})')*(r-rbias)];
            
        else 
           Jdot = (JC{i-1}-Jc{i})/h{i}
           rbias = Jc{i}*iM{i}*C{i}+Jc{i}*iM{i}*gv-Jdot*v;
           c=[c;(r-rbias)'*inv((Jc{i}*iM{i})*(Jc{i}*iM{i})')*(r-rbias)];
        end

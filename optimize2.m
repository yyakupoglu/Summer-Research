function [xOpt,F,xMul,Fmul,INFO] = optimize2()

global robot m N g
IC = [robot.homeConfiguration', zeros(1,3), robot.centerOfMass',zeros(1,6),[0 m*g 0], zeros(1, 6), 1]';
FC = [zeros(1,3), zeros(1,3), [1.25,0,0],zeros(1,6),[0 m*g 0], zeros(1, 6),1]';
xInit = guess_solution(IC,FC); 

numDecVar = length(xInit);
[xLow, xUpp] = setStateBoundaries(numDecVar);


usrfun = 'dynconsfunc';

Flow = [ 0;            %lowB bound of the cost (final time)
         zeros(133,1)]; %lowB of the dynamics of x y and v (each size N)

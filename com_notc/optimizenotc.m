function [trajKnots, xOpt] = optimizenotc()

global robot m N g simTime

IC = [robot.homeConfiguration',[0,0,0], robot.centerOfMass',zeros(1,3),[0 m*g 0], simTime/(N-1)]';
FC = [[45,0,90]*pi/180,[0,0,0],[0.813172798364530,0.883883476483184,0],zeros(1,3),[0 m*g 0],0]';
...
usrfun = 'consnotc';
conseqno=3; %how many constraint equations are there? check from cons file
...
Flow(8:18:end)=-1;%Change to fit DME constraint
Flow(9:18:end)=-1;
Flow(10:18:end)=-1;
...
Fupp(8:18:end)=1;%Change to fit DME constraint
Fupp(9:18:end)=1;
Fupp(10:18:end)=1;
